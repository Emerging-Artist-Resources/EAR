...
/** Extra top inset on pages 2+ before the first content row. */
...
function drawDefaultRow(ctx: DrawCtx, field: FiscalSponsorshipPdfFieldRow): DrawCtx {
...
function drawMultiselectRow(ctx: DrawCtx, field: FiscalSponsorshipPdfFieldRow): DrawCtx {
  const { contentX, labelColW, valueX, valueMaxW } = ctx.layout
...
function drawLongRow(ctx: DrawCtx, field: FiscalSponsorshipPdfFieldRow): DrawCtx {
...
function drawFieldRow(ctx: DrawCtx, field: FiscalSponsorshipPdfFieldRow): DrawCtx {
...
async function buildPdf(input: FiscalSponsorshipInquiryPdfInput, fontMode: "noto" | "standard"): Promise<Uint8Array> {
  ...
  const titleText = "Fiscal sponsorship inquiry"
...
export function buildFiscalSponsorshipInquiryPdfFileName(
  ...
  return `Fiscal-Sponsorship-Inquiry-${safe}-${y}-${m}-${d}.pdf`
}
...
export async function generateFiscalSponsorshipInquiryPdf(
  ...
  try {
    ...
    console.warn("Fiscal sponsorship inquiry PDF: Noto failed, using Helvetica", { error: notoErr })
