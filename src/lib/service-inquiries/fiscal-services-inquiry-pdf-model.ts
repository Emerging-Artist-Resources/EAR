...
export type FiscalServicesInquiryPdfInput = Omit<ServiceInquiryPdfInput, "documentTitle">
...
/**
 * Builds PDF structure mirroring the fiscal services inquiry form (contact + fiscal services sections).
 */
export function buildFiscalServicesInquiryPdfInput(params: {
  inquiryId: string
  ...
  const fiscalServicesRows = projectQuestions.map((q) =>
  ...
  return {
    ...
    sections: [
      ...
      { title: "Fiscal services", rows: fiscalServicesRows },
    ],
  }
}
