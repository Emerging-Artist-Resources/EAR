/**
 * Fiscal services inquiry — Postmark templates + PDF attachment.
 */

import { Attachment, TemplatedMessage } from "postmark"
import { postmarkClient } from "./postmark"

export const FISCAL_SERVICES_INQUIRY_TEMPLATE_ADMIN = "fiscal-service-inquiry-admin"
export const FISCAL_SERVICES_INQUIRY_TEMPLATE_CONFIRMATION = "fiscal-service-inquiry-confirmation"

type SendArgs = {
  to: string
  templateAlias:
    | typeof FISCAL_SERVICES_INQUIRY_TEMPLATE_ADMIN
    | typeof FISCAL_SERVICES_INQUIRY_TEMPLATE_CONFIRMATION
  templateModel: Record<string, unknown>
  pdfBytes: Uint8Array
  pdfFileName: string
}

export async function sendFiscalServicesInquiryTemplatedEmail({
  to,
  templateAlias,
  templateModel,
  pdfBytes,
