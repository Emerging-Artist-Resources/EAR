...
import { ParentEventOption } from "./types"
...
export function ParentSearch({ onSelect, selectedParentId: _selectedParentId }: ParentSearchProps) {
  ...
  const [results, setResults] = useState<ParentEventOption[]>([])
  ...
  const searchParents = useCallback(async (searchQuery: string) => {
    ...
    setIsSearching(true)
    try {
      const response = await fetch(`/api/admin/pieces/parents/search?q=${encodeURIComponent(searchQuery.trim())}`)
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <Input
        ...
        placeholder="Type to search for parent events..."
