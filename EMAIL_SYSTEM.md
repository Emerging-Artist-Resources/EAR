...
- **Template Variables**:
  - `{{first_name}}` - Greeting name (first name, or full name if single word, or `there` if missing; see Naming conventions)
  - `{{submitter_name}}` - Full name of the person who submitted
  - `{{listing_title}}` - Title of the listing
  - `{{cta_url}}` - Link to view the listing in dashboard
  - `{{feedback_url}}` - Link to the listing submission feedback form (Jotform; see `LISTING_SUBMISSION_FEEDBACK_FORM_URL` in `src/lib/config/site-contact.ts`)
...
- **Template Variables**:
  - `{{first_name}}` - Greeting name (see Naming conventions)
...
- **Alias**: `welcome-email`
- **Subject**: Welcome to Emerging Artist Resources (EAR)
- **Template Variables**:
  - `{{first_name}}` - Greeting name (see Naming conventions)
...
After a user successfully verifies their email, a one-time welcome email is sent:

**Flow:**
1. User clicks the verification link and lands on `/auth/callback`
2. `completeAuthCallbackClient()` establishes the Supabase session
3. The callback page fires `POST /api/auth/welcome-email` (fire-and-forget, does not block redirect)
4. `trySendWelcomeEmail()` atomically claims `profiles.welcome_email_sent_at`, then sends Postmark template `welcome-email`

**Location:** `src/app/auth/callback/page.tsx` (after successful verification)

**Service Function:** `src/lib/email/trySendWelcomeEmail.ts`

**API Route:** `src/app/api/auth/welcome-email/route.ts`

**Idempotency:**
- `profiles.welcome_email_sent_at` is set via an atomic `UPDATE ... WHERE welcome_email_sent_at IS NULL` before send
- Migration backfills this column for users who already had `auth.users.email_confirmed_at` set, so existing verified users do not receive a retroactive welcome email
- Re-clicks on the verification link or subsequent sign-ins do not send duplicate welcome emails

**Error Handling:**
- Email failures are logged; redirect after verification still succeeds
- If Postmark fails after the claim, `welcome_email_sent_at` is not rolled back (prevents duplicate sends on retry)
...
Use `greetingNameFromFullName()` from `src/lib/names/person-name.ts` whenever a Postmark template greets someone by name (`{{first_name}}`).

**Fallback order:** first whitespace-delimited token → full trimmed name (single-word names) → `"there"` (missing/blank).

| Variable | Use for |
|----------|---------|
| `{{first_name}}` | Greetings (`Hi {{first_name}},`) |
| `{{submitter_name}}`, `{{user_name}}` | Full name in body, subjects, or admin records |

**Postmark updates:** Change `listing-received` and `listing-updated` greetings from `{{submitter_name}}` to `{{first_name}}`. Other user-facing templates already use `{{first_name}}` for greetings.

### Standard Variables (for listing emails)

- `first_name` - String - Greeting name (see Naming conventions)
- `submitter_name` - String - Full name of the person who submitted
